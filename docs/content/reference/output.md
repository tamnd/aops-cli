...
describes what users get. Pick a format with `-o`, or let aops choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
aops <command> -o table   # aligned columns for reading
aops <command> -o jsonl   # one JSON object per line, for piping
aops <command> -o json    # a single JSON array
aops <command> -o csv     # spreadsheet friendly
aops <command> -o tsv     # tab-separated
aops <command> -o url     # just the URL column
aops <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
aops <command> --fields id,title,url
```
...
```bash
aops <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
aops <command>            # a table, because this is a terminal
aops <command> | wc -l    # JSONL, because this is a pipe
```
